...
function KosanIndex() {
	...
	const fetchData = async () => {
		...
		setLoading(true);
		await Api.get("/api/admin/houses", {
			headers: {
				//header Bearer + Token
	...
	const columns = [
		...
		},
		{
			...
			selector: row => {
				...
				const user = dataUser.find(user => user.id == row.user_id);

				// Jika user ditemukan, tampilkan namanya
			...
			cell: row => (
				<div className="gap-2 d-flex">
					<button className="btn btn-sm btn-success">
						<Link to={`/admin/kosan/edit/${row.id}`}>
